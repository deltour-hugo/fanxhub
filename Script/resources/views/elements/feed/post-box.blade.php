                ...
                <div class="d-flex justify-content-between">
                    ...
                    <div class="d-flex">
                        ...
                        @if(Auth::check() && $post->release_date && Auth::user()->id === $post->user_id && $post->is_scheduled)
                            ...
                            @endif
                        @endif
                        @if(Auth::check() && $post->price > 0)
                            <div class="pr-3 pr-md-3"><span class="badge badge-pill bg-gradient-faded-primary">{{ucfirst(__("PPV"))}}</span></div>
                        @endif

                        @if(Auth::check() && count($post->attachments) > 0)
                            <div class="pr-3 pr-md-3"><span class="badge badge-pill bg-gradient-faded-primary">@if (count($post->attachments) <= 1) {{ count($post->attachments) . __(' file') }} @else {{ count($post->attachments) .  __(' files') }} @endif</span></div>
                        @endif
                            ...
                            <div class="swiper-wrapper">
                                @foreach($post->attachments as $attachment)
                                    <div class="swiper-slide">
                                        @include('elements.feed.post-locked-box-media-wrapper',[
