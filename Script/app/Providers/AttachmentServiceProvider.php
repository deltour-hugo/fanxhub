...
class AttachmentServiceProvider extends ServiceProvider
{
    ...
    public static function createAttachment($file, $directory, $generateThumbnail)
    {
        ...
        if (self::getAttachmentType($fileExtension) == 'image') {
            $jpgImage = Image::make($baseImage);
            $jpgImage->fit($jpgImage->width(), $jpgImage->height())->blur(100)->orientate();

            $jpgImage2 = Image::make($baseImage);
            $jpgImage2->fit($jpgImage2->width(), $jpgImage2->height())->orientate();
            ...
            if($fileExtension == 'gif'){
                ...
                $fileExtension = 'jpg';
                $fileContent = $file;
                $hsrd = sha1('abeautifulkey');
                $blurredFilePath = $directory.'/'. $hsrd . $fileId.'.'.$fileExtension;
                // Uploading to storage
                $storage->put($blurredFilePath, $fileContent, 'public');

                $jpgImage2->encode('jpg', 100);
                $file2 = $jpgImage2;
                $fileExtension = 'jpg';
                $fileContent = $file2;
                $filePath = $directory.'/' .$fileId.'.'.$fileExtension;
        ...
        if ($generateThumbnail && self::getAttachmentType($fileExtension) === 'image') {
            ...
            $img2 = Image::make($baseImage);
            $img2->fit(150, 150, function ($constraint) {
                $constraint->aspectRatio();
            });
            $img2->encode('jpg', 100);
            ...
            // Uploading to storage
            $storage->put($thumbnailfilePath, $img2, 'public');

            $otherImg = Image::make($baseImage);
            $otherImg->fit(150, 150, function ($constraint) {
                $constraint->aspectRatio();
            });
            $otherImg->blur(100);
            $otherImg->encode('jpg', 100);
            $thumbnailfilePath = $thumbnailDir.'/' . $hsrd .$fileId.'.jpg';
            // Uploading to storage
            $storage->put($thumbnailfilePath, $otherImg, 'public');

            $thumbnailDir = $directory.'/'.'250X250';
            $thumbnailfilePath = $thumbnailDir . '/' . $fileId . '.jpg';
            // Uploading to storage
            $storage->put($thumbnailfilePath, $img2, 'public');

            $otherImg = Image::make($baseImage);
            $otherImg->fit(250, 250, function ($constraint) {
                $constraint->aspectRatio();
            });
            $otherImg->blur(100);
            $otherImg->encode('jpg', 100);
        ...
        !isset($blurredFilePath) ? $blurredFilePath = null : $blurredFilePath;

        $attachment = Attachment::create([
            'id' => $fileId,
            'filename' => $filePath,
            'blurredFilename' => $blurredFilePath,
            'user_id' => Auth::id(),
            'type' => $fileExtension,
    ...
    public static function removeAttachment($attachment)
    {
        ...
        !isset($attachment->blurredFilename) ? $storage->delete($attachment->filename) : $storage->delete([$attachment->filename, $attachment->blurredFilename]);
        ...
        if (self::getAttachmentType($attachment->type) == 'image') {
            ...
            $blurredThumbnailPath = self::getBlurredThumbnailFilenameByAttachmentAndResolution($attachment, $width = 150, $height = 150);

            if ($thumbnailPath != null) {
                $storage->delete([$thumbnailPath, $blurredThumbnailPath]);
            }
        }
    }
    ...
    /**
     * Returns file blurred thumbnail path, by resolution.
     *
     * @param $attachment
     * @param $width
     * @param $height
     * @return string|string[]
     */
    private static function getBlurredThumbnailFilenameByAttachmentAndResolution($attachment, $width, $height, $basePath = 'posts/images/')
    {
        return str_replace($basePath, $basePath.$width.'X'.$height.'/', $attachment->blurredFilename);
    }
     ...
     * @return string
     */
    public static function getFileBlurredPathByAttachment($attachment)
    {
        ...
        if ($attachment->driver == Attachment::S3_DRIVER) {
            if (getSetting('storage.aws_cdn_enabled') && getSetting('storage.aws_cdn_presigned_urls_enabled')) {
                $fileUrl = self::signAPrivateDistributionPolicy(
                    'https://'.getSetting('storage.cdn_domain_name').'/'.$attachment->blurredFilename
                );
            } elseif (getSetting('storage.aws_cdn_enabled')) {
                $fileUrl = 'https://'.getSetting('storage.cdn_domain_name').'/'.$attachment->blurredFilename;
            } else {
                $fileUrl = 'https://'.getSetting('storage.aws_bucket_name').'.s3.'.getSetting('storage.aws_region').'.amazonaws.com/'.$attachment->blurredFilename;
            }
        }
        elseif ($attachment->driver == Attachment::WAS_DRIVER || $attachment->driver == Attachment::DO_DRIVER) {
            $fileUrl = Storage::url($attachment->blurredFilename);
        }
        elseif($attachment->driver == Attachment::MINIO_DRIVER){
            $fileUrl = rtrim(getSetting('storage.minio_endpoint'), '/').'/'.getSetting('storage.minio_bucket_name').'/'.$attachment->blurredFilename;
        }
        elseif($attachment->driver == Attachment::PUSHR_DRIVER){
            $fileUrl = rtrim(getSetting('storage.pushr_cdn_hostname'), '/').'/'.$attachment->blurredFilename;
        }
        elseif ($attachment->driver == Attachment::PUBLIC_DRIVER) {
            $fileUrl = Storage::disk('public')->url($attachment->blurredFilename);
        }
